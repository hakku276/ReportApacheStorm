...
\begin{abstract}
In today's world everyone is connected to this ever increasing network of computers be it via a Personal computer, a smart phone or an Embedded Device such as your smart home appliances. These devices generate huge amounts of data every instance, that can be analyzed to uncover patterns, some of which can be used to improve businesses as well. In this report we talk about such scenario and the available tools which allows us to process large data streams with minimum latency. Specifically Apache Storm, a publicly available Stream Processing Tool. We will dive into the concepts of Storm, how one can design usable systems with it and also compare storm to the other available tools.
\end{abstract}
