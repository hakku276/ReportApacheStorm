...
\subsection{Stream Processing Models}
There are two types of stream processing models, namely continuous flow model and discretized stream model. \cite{streaming}

\subsubsection{Continuous Stream Processing}


\subsubsection{Discretized Stream Processing}
...
The physical high level architecture of Storm as seen in figure \ref{fig:physicalarc} consists of a single master node and multiple worker nodes. The master node(see later) is the leader of the cluster and is responsible in short for the management of the different parts of the topology. There can only be one master node in a cluster. The worker node on the other hand is responsible for the actual execution of tasks supplied by the user. A user task is defined in storm as a Topology which actually is a directed graph. The vertices of the graph represent the data processing being done and the edges the flow of data. A data stream passed through the topology yields the required processing result as defined by the user.

\begin{figure}
  \begin{center}
    \includegraphics[width=.7\textwidth]{arch.png}
    \caption{High Level Storm Physical Architecture}
    \label{fig:physicalarc}
   \end{center}
\end{figure}

The logical or the runtime architecture of storm is defined within a topology. As seen in figure \ref{fig:logicalarch} shows an example of a topology and consists of vertices and edges, the vertices represent point where either data is generated or some processing to the existing data is done. The edges represent the flow of data, and in a topology, the data is represented in the form of tuples. A vertex can in storm be either a spout or a bolt. A spout is the source of tuples and it injects tuples into the topology, wheras a bolt is a vertice within a topology where the injected tuples are processed.

\begin{figure}
  \begin{center}
    \includegraphics[width=.7\textwidth]{arch.png}
    \caption{High Level Storm run-time or Logical Architecture}
    \label{fig:logicalarch}
   \end{center}
\end{figure}

The logical and physical architectures meet while being deployed in a cluster, the worker nodes are responsible for executing the topology. Every worker spawns a number of processes which run topologies. See later for detailed explanation on how topologies can be divided and executed within a storm cluster.

\subsubsection{Storm Components}
\paragraph{Master node} also called the \textbf{Nimbus} is responsible for the distribution and coordination of the execution of topologies. The Nimbus is the first point where the user submits the defined topology, which is then distributed among the worker nodes. The user submits the code as a jar file to the nimbus, which in turn uses a combination of local disk and the zookeeper to store the topology defined in the jar file.\cite{stormtwitter} The topology thrift objects are stored in the zookeeper, whereas the code itself is stored in the local disk of nimbus.
\paragraph{Worker Node} is responsible for all the heavy lifting required for processing the incoming data stream. A worker node maintains a \textbf{Supervisor} process and multiple worker processes. The number of  worker processes depends upon the storm configurations. A worker process once spawned, starts working for a single topology alone and not for a multiple number of topologies. It creates threads called \textbf{Executors} which take up a tasks to be performed. A task for an executor is the vertex of the Directed graph, ie. either a bolt or a spout. This can be observed in the \ref{fig:workerarch}, where a worker node has multiple worker processes, which in turn has multiple executors running different tasks. 
Even though storm can run multiple topologies, different processes run separate topologies. 

The introduction of tasks enables a parallel execution of a number of same bolt or spout node of a topology, whereas the introduction of executors enables us to run different parts of the topology namely bolts and spouts in parallel.\cite{stormtwitter}. It is worth noting that these different parts of the topology, namely bolts and spouts can be executing within a single worker process in different executors, or can also be in different executors across different worker nodes.
...
A stream is a continuous flow of data from one point to the other. A Stream Processing system works on these data streams, the work may just be simple transformations, but could also be as complex as predictive analysis. A stream processing system depending on the nature of application has different requirements.
...
As discussed above, a data processing stream has different requirements and according to these various requirements, the System can be modified to suit these needs. The major two System architectures currently available are the Lambda Architecture and the Lambda-less (or Kappa) Architecture.
...
A storm topology design with the spouts and bolts and affects the overall system thus should be properly considered. A spout should inject data into the topology from external sources whereas the bolt should be a smallest implementable task. A storm topology should also be considered with various considerations in mind:
...
\subsubsection{Resilience:} Apache storm includes APIs which allow users to develop solutions which are resilient in nature. Using the acker bolt as shown in figure \ref{fig:acker}, users can track the progress of the tuple injected into the system. Thus, writing a spout which can replay the tuple in case of failures allows us to guarantee the processing of data. For additional details please follow Section \ref{sec:resilience}.
...
Stream processing is a topic under constant focus at present and a lot of tools as well as models have been designed and created to support it. This section makes a comparison of the available widely accepted tools today. The available tools are mentioned below in brief such as:
